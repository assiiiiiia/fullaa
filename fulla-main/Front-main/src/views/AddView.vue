...
export default {
  ...
  // Format `due_date` by combining `date` and `time`
  let dueDate;
  if (this.task.time) {
    // If time is provided, combine date and time
    dueDate = `${this.task.date} ${this.task.time}:00`;
  } else {
    // If no time is provided, use the date only (no time part)
    dueDate = this.task.date; // Just the date, no time added
  }
  ...
  const formattedTask = {
    ...
    due_date: dueDate, // Send the correctly formatted date
    priority: importanceMap[this.task.importance], // Map importance
  };
...
,
    ...
    addAnotherTask() {
      // Fermer la popup et rouvrir le formulaire
